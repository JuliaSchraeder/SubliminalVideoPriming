...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/julia/Desktop/Promotion/DFG_Projekt/Paradigmen/VideoMask/Rating.py',
...
slider = visual.Slider(win=win, name='slider',
    ...
    flip=False, depth=0)
text_slider = visual.TextStim(win=win, name='text_slider',
    text='traurig                       glücklich',
    font='Arial',
...
trialComponents = [slider, text_slider]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if text_slider.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > text_slider.tStartRefresh + 1.0-frameTolerance:
            # keep track of stop time/frame for later
            text_slider.tStop = t  # not accounting for scr refresh
            text_slider.frameNStop = frameN  # exact frame index
            win.timeOnFlip(text_slider, 'tStopRefresh')  # time at next scr refresh
            text_slider.setAutoDraw(False)
...
thisExp.addData('text_slider.started', text_slider.tStartRefresh)
thisExp.addData('text_slider.stopped', text_slider.tStopRefresh)
