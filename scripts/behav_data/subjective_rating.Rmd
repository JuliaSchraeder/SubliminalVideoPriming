...
**Two Peaks**: A bimodal distribution has two distinct peaks or clusters. For example, in your case, one peak might be around a rating of 3, and another around 6.
...
ggplot(emm_group, 
  ...
  ## Adding significance annotations (asterisks)
  #geom_text(data = significant_comparisons, 
            #aes(x = as.numeric(factor(group1)) + 0.5,  
                #y = max(emm_group$emmean + emm_group$SE) + 3.3, 
                #label = asterisks), 
            ## Position the asterisks
            #vjust = -0.5, size = 7, inherit.aes = FALSE) +  

  ## Adding lines to show significant comparisons
  #geom_segment(data = significant_comparisons,
               #aes(x = group1, xend = group2,
                   #y = max(emm_group$emmean + emm_group$SE) +3.7, 
                   #yend = max(emm_group$emmean + emm_group$SE) + 3.7),
               #color = "black", size = 0.8, inherit.aes = FALSE)
ggsave("Rating_Group.tiff")
```
...
participant_aggregates$condition <- factor(participant_aggregates$condition, levels = condition_levels)
pairs_condition$condition <-factor(pairs_condition$condition, levels = condition_levels)
```
...
ggsave("Rating_Interaction.tiff", plot = interaction_plot, width = 10, height = 6, dpi = 300)
